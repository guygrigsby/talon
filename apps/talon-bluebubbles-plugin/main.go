// talon-bluebubbles-plugin is a Go subprocess plugin serving the
// BlueBubbles iMessage channel through talon's gRPC plugin protocol.
// Native counterpart to the openclaw-bundled extensions/bluebubbles
// (TS); same channel name (channels.bluebubbles.*) so flipping
// between the two is a config swap.
//
// Wire shape:
//
...
//   - Initialize: offers_channels=["bluebubbles"], offers_tools=
//     ["bluebubbles_send"], no Host-side needs (auth comes via
//     StartChannel.channel_config bytes — the host passes the
//     channels.bluebubbles.* sub-tree of merged config).
//   - StartChannel("bluebubbles", {serverUrl, password, webhookPort,
//     ...}): spins up an HTTP listener on webhookPort. The user's
//     BlueBubbles server is configured (via its admin UI) to POST
//     'new-message' events to http://<host>:<port><webhookPath>;
//     each event becomes one IncomingChannelMessage on the gRPC
//     stream. Honors stream context for clean shutdown.
//   - SendChannelMessage: POST <serverUrl>/api/v1/message/text with
//     chatGuid=room_id (round-tripped from inbound) + message text.
//   - bluebubbles_send tool: same wire path as SendChannelMessage,
//     but accepts an explicit chat_guid for proactive messaging when
//     the agent isn't replying to a known room.
//
// V1 deferred (tracked in talon-n5n description): reactions, edits,
// unsend, attachments, group enrichment, catchup, send wizard parity.
//
// Build:
//
//	go build -o /tmp/talon-bluebubbles ./apps/talon-bluebubbles-plugin
package main
...
func main() {
	...
	log := slog.With("plugin", "bluebubbles")
	...
	plug := &bluebubblesPlugin{http: &http.Client{Timeout: defaultSendTimeout + 10*time.Second}}
