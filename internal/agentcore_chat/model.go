...
// secrets through the same chain the legacy stack uses, and adapts
...
// provider-specific compatibility gaps remain on the legacy path.
//
...
//  2. per-agent `agents.list[<i>].model` (legacy shape: a bare
