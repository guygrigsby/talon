...
func TestResolvePluginCmd_AsIsWhenExists(t *testing.T) {
	...
	got, err := resolvePluginCmd("p", []string{bin, "--flag"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolvePluginCmd_FallsBackToSibling(t *testing.T) {
	...
	got, err := resolvePluginCmd("p", []string{configured})
	if err != nil {
		t.Fatal(err)
	}
	if got[0] != sibling {
...
func TestResolvePluginCmd_EmptyCmd(t *testing.T) {
	if _, err := resolvePluginCmd("p", nil); err == nil || !strings.Contains(err.Error(), "empty Cmd") {
