import { n as getMSTeamsRuntime, t as runtime_api_exports } from "./runtime-api-B8iSyYot.js";
import { A as ATTACHMENT_TAG_RE, B as isLikelyImageAttachment, C as loadMSTeamsSdkWithAuth, D as formatMSTeamsSendErrorHint, E as classifyMSTeamsSendError, F as estimateBase64DecodedBytes, G as resolveAttachmentFetchPolicy, H as isUrlAllowed, I as extractHtmlFromAttachment, J as safeFetchWithPolicy, K as resolveMediaSsrfPolicy, L as extractInlineImageCandidates, M as IMG_SRC_RE, N as applyAuthorizationHeaderForUrl, O as formatUnknownError, P as encodeGraphShareId, R as inferPlaceholder, S as createMSTeamsTokenProvider, T as ensureUserAgentHeader, U as normalizeContentType, V as isRecord, W as readNestedString, X as tryBuildGraphSharesUrlForSharedLink, Y as safeHostForUrl, _ as resolveMSTeamsStorePath, a as fetchGraphJson, b as createBotFrameworkJwtValidator, h as resolveMSTeamsCredentials, j as GRAPH_ROOT, q as resolveRequestUrl, w as buildUserAgent, x as createMSTeamsAdapter, z as isDownloadableAttachment } from "./graph-users-B1f6vAra.js";
import { c as resolveMSTeamsUserAllowlist, s as resolveMSTeamsChannelAllowlist } from "./resolve-allowlist-BavrSb7x.js";
import { a as resolveMSTeamsRouteConfig, i as resolveMSTeamsReplyPolicy, n as resolveMSTeamsAllowlistMatch, t as isMSTeamsGroupAllowed } from "./policy-BwUzhs5c.js";
import { C as readJsonFile, S as createMSTeamsConversationStoreFs, T as writeJsonFile, _ as buildFileInfoCard, b as createMSTeamsPollStoreFs, c as renderReplyPayloadsToMessages, d as withRevokedProxyFallback, f as resolveGraphChatId, g as removePendingUploadFs, h as getPendingUploadFs, l as sendMSTeamsMessages, m as removePendingUpload, p as getPendingUpload, s as buildConversationReference, u as AI_GENERATED_ENTITY, v as parseFileConsentInvoke, w as withFileLock, x as extractMSTeamsPollVote, y as uploadToConsentUrl } from "./probe-BPq3ou5k.js";
...
async function runFeedbackReflection(params) {
	...
	try {
		await (0, runtime_api_exports.dispatchReplyFromConfigWithSettledDispatcher)({
			ctxPayload,
			cfg,
...
async function resolveMSTeamsSenderAccess(params) {
	...
	const pairing = (0, runtime_api_exports.createChannelPairingController)({
		core: getMSTeamsRuntime(),
		channel: "msteams",
		accountId: runtime_api_exports.DEFAULT_ACCOUNT_ID
	});
	const dmPolicy = msteamsCfg?.dmPolicy ?? "pairing";
	const storedAllowFrom = await (0, runtime_api_exports.readStoreAllowFromForDmPolicy)({
		provider: "msteams",
	...
	const resolvedAllowFromLists = (0, runtime_api_exports.resolveEffectiveAllowFromLists)({
		allowFrom: configuredDmAllowFrom,
		groupAllowFrom,
		storeAllowFrom: storedAllowFrom,
		dmPolicy
	});
	const defaultGroupPolicy = (0, runtime_api_exports.resolveDefaultGroupPolicy)(params.cfg);
	...
	const allowNameMatching = (0, runtime_api_exports.isDangerousNameMatchingEnabled)(msteamsCfg);
	...
	const senderGroupPolicy = channelGate.allowlistConfigured && effectiveGroupAllowFrom.length === 0 ? groupPolicy : (0, runtime_api_exports.resolveSenderScopedGroupPolicy)({
		groupPolicy,
		groupAllowFrom: effectiveGroupAllowFrom
	});
	const access = (0, runtime_api_exports.resolveDmGroupAccessWithLists)({
	...
	return {
		...
		channelGate,
		access,
		senderGroupAccess: (0, runtime_api_exports.evaluateSenderGroupAccessForPolicy)({
			groupPolicy,
			groupAllowFrom: effectiveGroupAllowFrom,
			senderId,
...
function buildMSTeamsMediaPayload(mediaList) {
	return (0, runtime_api_exports.buildMediaPayload)(mediaList, { preserveMediaTypeCardinality: true });
}
//#endregion
...
function createMSTeamsReplyDispatcher(params) {
	...
	*/
	...
	const { onModelSelected, typingCallbacks, ...replyPipeline } = (0, runtime_api_exports.createChannelReplyPipeline)({
		cfg: params.cfg,
		agentId: params.agentId,
		channel: "msteams",
		...
		typing: {
			...
			onStartError: (err) => {
				(0, runtime_api_exports.logTypingFailure)({
	...
	const mediaMaxBytes = (0, runtime_api_exports.resolveChannelMediaMaxBytes)({
		cfg: params.cfg,
...
function createMSTeamsReactionHandler(deps) {
	...
	return async function handleReaction(context, direction) {
		...
		const defaultGroupPolicy = (0, runtime_api_exports.resolveDefaultGroupPolicy)(cfg);
		if (isDirectMessage && msteamsCfg) {
			const access = (0, runtime_api_exports.resolveDmGroupAccessWithLists)({
				isGroup: false,
				dmPolicy,
				...
				isSenderAllowed: (allowFrom) => resolveMSTeamsAllowlistMatch({
					...
					senderId,
					senderName,
					allowNameMatching: (0, runtime_api_exports.isDangerousNameMatchingEnabled)(msteamsCfg)
				}).allowed
			});
...
const MSTEAMS_WEBHOOK_MAX_BODY_BYTES = runtime_api_exports.DEFAULT_WEBHOOK_MAX_BODY_BYTES;
async function monitorMSTeamsProvider(opts) {
	...
	const resolveAllowlistUsers = async (label, entries) => {
		...
		(0, runtime_api_exports.summarizeMapping)(label, resolved.filter((entry) => entry.resolved && entry.id).map((entry) => `${entry.input}→${entry.id}`), unresolved, runtime);
		return {
			additions,
			unresolved
		};
	...
	try {
		...
		if (allowEntries.length > 0) {
			...
			allowFrom = (0, runtime_api_exports.mergeAllowlist)({
				existing: allowFrom,
				additions
			});
		}
		if (Array.isArray(groupAllowFrom) && groupAllowFrom.length > 0) {
			...
			if (groupEntries.length > 0) {
				...
				groupAllowFrom = (0, runtime_api_exports.mergeAllowlist)({
		...
		if (teamsConfig && Object.keys(teamsConfig).length > 0) {
			...
			if (entries.length > 0) {
				...
				(0, runtime_api_exports.summarizeMapping)("msteams channels", mapping, unresolved, runtime);
			}
		}
	} catch (err) {
	...
	await (0, runtime_api_exports.keepHttpServerTaskAlive)({
		server: httpServer,
