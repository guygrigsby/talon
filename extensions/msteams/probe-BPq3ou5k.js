...
async function getMimeType(url) {
	...
	return await (0, runtime_api_exports.detectMime)({ filePath: url }) ?? "application/octet-stream";
}
/**
...
async function extractFilename(url) {
	if (url.startsWith("data:")) {
		...
		const ext = (0, runtime_api_exports.extensionForMime)(mime) ?? ".bin";
		return `${mime.startsWith("image/") ? "image" : "file"}${ext}`;
	}
	try {
		const pathname = new URL(url).pathname;
		const basename = path.basename(pathname);
		const existingExt = (0, runtime_api_exports.getFileExtension)(pathname);
		...
		return (0, runtime_api_exports.extractOriginalFilename)(url);
	}
}
/**
...
async function sendMessageMSTeams(params) {
	...
	if (mediaUrl) {
		const media = await (0, runtime_api_exports.loadOutboundMediaFromUrl)(mediaUrl, {
...
function readStringArray(value) {
	...
	const out = (0, runtime_api_exports.normalizeStringEntries)(value);
