...
import { n as isRepairableSecretStorageAccessError, r as MATRIX_IDB_PERSIST_INTERVAL_MS, t as MatrixRecoveryKeyStore } from "./recovery-key-store-D6keko6t.js";
...
async function loadMatrixCryptoRuntime() {
	matrixCryptoRuntimePromise ??= import("./crypto-runtime-DF1p2f2C.js").then((runtime) => {
...
var MatrixClient = class {
	...
	async getOwnDeviceVerificationStatus() {
		...
		const serverDeviceKnown = deviceId ? ownDevices?.some((device) => device.deviceId === deviceId) ?? null : null;
		return {
			...deviceVerification,
	...
	async verifyWithRecoveryKey(rawRecoveryKey) {
		...
		try {
			...
			const stagedRecoveryKeyUnlockedBackup = stagedRecoveryKeyUsed && !stagedRecoveryKeyRejectedBySecretStorage && !stagedRecoveryKeyConfirmedBySecretStorage && !backupUsableBeforeStagedRecovery && backupUsable;
			const storedRecoveryKeyMatches = this.recoveryKeyStore.getRecoveryKeySummary()?.encodedPrivateKey?.trim() === trimmedRecoveryKey;
			const stagedRecoveryKeyValidated = stagedRecoveryKeyUsed && (stagedRecoveryKeyConfirmedBySecretStorage || stagedRecoveryKeyUnlockedBackup) || storedRecoveryKeyMatches && backupUsable;
