...
async function buildInboundContext(event, deps) {
	...
	if (selfEchoAccess) {
		log?.info(`Blocked qqbot inbound self-echo: reasonCode=${selfEchoAccess.reasonCode} msgIdx=${event.msgIdx ?? ""} senderId=${normalizeQQBotSenderId(event.senderId)} accountId=${account.accountId} isGroup=${isGroupChat}`);
...
function resolveBotSelfEchoAccess(event, accountId) {
	const currentMsgIdx = event.msgIdx?.trim();
	if (!currentMsgIdx) return null;
	const refEntry = getRefIndex(currentMsgIdx);
	if (refEntry?.isBot !== true || refEntry.senderId !== accountId) return null;
	return {
		decision: "block",
